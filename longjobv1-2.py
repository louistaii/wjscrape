...
async def scrape_all_pages(pages):
    """
    Fetches all MAX_PAGES pages concurrently -- since the page count is
    fixed and known ahead of time, there's no need to discover it
    sequentially or stop early on a short/duplicate page.
    ...
    for i in range(MAX_PAGES):
        tasks.append(asyncio.create_task(scrape_page(pages[i], urls[i], page_num=i + 1)))
        if i < MAX_PAGES - 1:
    ...
    for page_num, (items, status) in enumerate(results, start=1):
        if status != "ok":
...
async def launch_browser():
    ...
    # sb_cdp.Chrome() runs its own event loop internally (loop.run_until_complete),
    # which crashes with "Cannot run the event loop while another loop is
    # running" if called directly from inside our already-running asyncio
    # loop. Running it in a separate thread via to_thread gives it a thread
    # of its own with no conflicting loop.
...
async def main():
    ...
    while datetime.now(SGT) < stop_time:
        ...
        all_items, block_reason = await scrape_all_pages(pages)
