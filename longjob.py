...
POLL_INTERVAL = 60  # seconds, target time between cycle starts

# Only send a Telegram alert when the set of in-stock items actually changes
ALERT_ON_NO_CHANGE_EVERY = 10  # still send a heartbeat every N cycles
...
# Small pool of realistic viewport sizes; picked randomly per run instead of using a single fixed 1920x1080 every time.
VIEWPORTS = [
...
# Injected before any page script runs, to patch over the most common automation tells that bot-detection scripts check for.
STEALTH_INIT_SCRIPT = """
...
def scrape_page(page, url: str):
    ...
    # Give Lazada's normal page requests time to finish, with some jitter instead of a fixed wait every single time.
    page.wait_for_timeout(random.randint(4000, 7000))

    # Small human-like scroll, some detection scripts check for any interaction/movement at all before trusting a session.
    try:
...
def new_context(browser):
    """Creates a fresh browser context with a random viewport and
    the stealth patches applied. Used both at startup and if we
    ever need to recreate the session (e.g. after a proxy switch)."""
...
if __name__ == "__main__":
    ...
    startup_delay = random.uniform(5, 18)
    ...
    with sync_playwright() as p:
        ...
        while datetime.now(SGT) < stop_time:
            ...
            if not all_items:
                empty_streak += 1
                print(f"  Empty capture (streak: {empty_streak})")
                ...
                if (
                    ...
                    send_telegram_message(
                        "⚠️ No product data captured for "
            ...
            # Check how long scraping took, and subtract it from 60 seconds.
            elapsed = time.time() - cycle_start
            remaining = max(0, POLL_INTERVAL - elapsed)
            sleep_time = remaining + random.uniform(0, 5)
